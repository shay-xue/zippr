...
<body>
  <main>
    ...
    <div id="status">Waiting for keyboard focus.</div>
...
def make_handler(key_state: BrowserKeyState) -> type[BaseHTTPRequestHandler]:
    class WebTeleopHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            message = "Active keys: " + (", ".join(sorted(keys)) if keys else "(none)")
...
def serve(host: str = HOST, port: int = PORT) -> int:
    ...
    runner = WebTeleopRunner(controller=controller, settings=settings, key_state=key_state)

    server = ThreadingHTTPServer((host, port), make_handler(key_state))
    runner.start()
